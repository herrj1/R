...
stuff1<-cbind(stuff1,stuff1.label) #combine label column 
names(stuff1)<-c("weight","height","label") #assign column labels to top row 

stuff2<-read.table("stuff2.txt",header=TRUE,sep=",")
stuff2.label<-rep(-1,100)
stuff2<-cbind(stuff2,stuff2.label) #combine label column
names(stuff2)<-c("weight","height","label") #assign column labels to top row 
